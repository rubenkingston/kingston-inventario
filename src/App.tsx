...
import { Mic, MapPin, Truck, Search, Plus, CheckCircle2, Circle, Pencil, Layers, Monitor, Lightbulb, Armchair, Box, ChevronDown, ChevronUp, Download, Copy, QrCode, ShieldCheck, X } from 'lucide-react';
import { supabase } from './supabase';
import { Button, Input, Dialog, DialogContent, DialogHeader, DialogTitle, Badge } from './ui';
import { QRCodeCanvas } from 'qrcode.react';

// Categorías configuradas según el diseño Navy
const CATEGORIES = [
...
export default function App() {
  ...
  // Función handleUpdate corregida para resolver error ts(2304)
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans pb-20">
      ...
      <main className="container mx-auto px-4 mt-6">
        {activeTab === 'equipos' || activeTab === 'inventario' ? (
                ...
                return (
                  <div key={item.id} className={`p-5 rounded-xl border transition-all ${isInTruck ? 'bg-green-900/20 border-green-500' : isSelected ? 'border-blue-500 bg-blue-900/10' : 'bg-slate-800/50 border-slate-700'}`}>
                    <div className="flex flex-col md:flex-row gap-6">
                      <div className="flex items-start gap-4"><button disabled={isInTruck} onClick={() => setSelectedIds(p => isSelected ? p.filter(id => id !== item.id) : [...p, item.id])}>{isSelected ? <CheckCircle2 className="text-blue-500" /> : <Circle className="text-slate-600" />}</button><div className="p-3 rounded-lg bg-slate-700 text-slate-300"><CatIcon size={24}/></div></div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3"><h3 className="text-lg font-bold text-white truncate">{item.name}</h3><Badge className={item.status === 'operativo' ? 'bg-green-500/20 text-green-400 border-0' : 'bg-red-500/20 text-red-400 border-0'}>{item.status}</Badge></div>
              ...
              <div key={loc.id} className="bg-slate-800/50 border border-slate-700 rounded-lg overflow-hidden">
                <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-slate-800" onClick={() => setExpandedLocation(expandedLocation === loc.id ? null : loc.id)}>
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      <button onClick={async (e) => {e.stopPropagation(); if(confirm('¿Borrar?')) { await supabase.from('locations').delete().eq('id', loc.id); fetchData(); }}} className="text-red-500/70 hover:text-red-500"><X size={16}/></button>
                    </div>
                </div>
          ...
          <div className="text-center p-12 border-2 border-dashed border-slate-800 rounded-xl"><ShieldCheck size={48} className="mx-auto text-blue-500 mb-4 opacity-50"/><h2 className="text-xl font-bold text-white">Panel Admin Kingston</h2></div>
        )}
      </main>
      ...
      <Dialog open={isTruckOpen} onOpenChange={setIsTruckOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white">
          <DialogHeader><DialogTitle className="flex justify-between items-center"><span>Camión</span><Button variant="ghost" size="sm" className="text-red-400 text-xs" onClick={() => setTruckIds([])}>Vaciar Camión</Button></DialogTitle></DialogHeader>
        ...
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-2xl">
          ...
          {editingItem && (
            <div className="grid gap-4 py-4">
              ...
              <select className="bg-slate-800 p-2 rounded text-white border border-slate-700 outline-none" value={editingItem.status} onChange={e => setEditingItem({...editingItem, status: e.target.value})}><option value="operativo">Operativo</option><option value="reparacion">En Reparación</option></select>
      ...
      <Dialog open={isLocModalOpen} onOpenChange={setIsLocModalOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white">
          <DialogHeader><DialogTitle>Gestionar Ubicación</DialogTitle></DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <Input className="bg-slate-800 border-slate-700" placeholder="Dirección completa" value={newLoc.address} onChange={e => setNewLoc({...newLoc, address: e.target.value})} />
